...
To use OpenAI LLM models, set the `OPENAI_API_KEY` environment variable. You can obtain the OpenAI API key from the [OpenAI Platform](https://platform.openai.com/account/api-keys).

Mem0 can also use ChatGPT Plus/Pro Codex OAuth credentials created by Pi. Run `/login` in Pi, select ChatGPT Plus/Pro (Codex), then set `use_codex_oauth=True` in the OpenAI LLM config or export `OPENAI_USE_CODEX_OAUTH=true`. Mem0 reads Pi's `~/.pi/agent/auth.json` by default and calls the Codex Responses backend instead of the standard OpenAI API-key endpoint.
...
config = {
    "llm": {
        ...
        "config": {
            ...
            # Optional: override Pi's default ~/.pi/agent/auth.json path.
            # "codex_auth_file": "~/.pi/agent/auth.json",
        }
    }
}
